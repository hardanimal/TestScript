...
allreglist = {"CAPMEAS":0x03,"RESET_SYS":0x04,"RESET_MIN_MAX":0x05,
	            ...
	            "ES_TECH":0x15,"ES_LIFETIME":0x16,
	            "HWREADY":0x20,"GTG":0x21,"GTG_WARN":0x22,"PGEMSTAT":0x23,
	            "ES_TEMP0":0x24,"ES_TEMP1":0x25,"VIN":0x26,"VCAP":0x27,"CHG_TIME_LSB":0x28,"CHG_TIME_MSB":0x2B,
...
def get_temp_his():
    ...
    for i in range(16):
        (ret1, rdata1) = read_ee_byte(0x020A + i * 2)
        (ret2, rdata2) = read_ee_byte(0x020A+ i * 2 + 1)
        if (ret1 != 0 or ret2 != 0):
            return -1
...
def get_model ():
    ...
    print "Amber Model Number:%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c" % (j[0],j[1],j[2],j[3],j[4],j[5],j[6],j[7],j[8],j[9],j[10],j[11],j[12],j[13],j[14],j[15])
...
def get_fw_ver ():
    ...
    print "Amber FW version: V%x.%x" % (rdata1,rdata2)
...
def get_hw_ver ():
    ...
    print "Amber HW version: %x" % rdata1
...
def get_cappn ():
    ...
    for i in range(11):
        ...
        (ret13, rdata13) = read_ee_byte(0x002c )
        (ret14, rdata14) = read_ee_byte(0x002d )
        (ret15, rdata15) = read_ee_byte(0x002e )
        (ret16, rdata16) = read_ee_byte(0x002f )
     
        if (ret1 != 0 or ret2 != 0 or ret3 != 0 or ret4 != 0 or ret5 != 0 or ret6 != 0 or ret7 != 0 or ret8 != 0 ):
            return -1
       
    print "Capacitor part number: %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c" % (rdata1,rdata2,rdata3,rdata4,rdata5,rdata6,rdata7,rdata8,rdata9,rdata10,rdata11,rdata12,rdata13,rdata14,rdata15,rdata16)
